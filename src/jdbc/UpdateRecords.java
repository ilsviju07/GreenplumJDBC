...
public class UpdateRecords
{
	...
	public UpdateRecords(String[] args) throws SQLException
	{
		myArgs = args;
	...
	public int updateFromTempTable (String tableName) throws SQLException
	{
		int updates = 0;
		...
		for (int i=0; i<txndts.length; i++)
		{
			...
			int myUpdates = gpcon2.stmt.executeUpdate(sql);
			updates += myUpdates;
			System.out.println ("Updated " + myUpdates + " records for txndt = " + txndts[i]);
		}
		
		return updates;
	}

	public int updateRecords(boolean usegpfdist) throws SQLException, IOException, InterruptedException
	{
		int cntInserted = 0;
		...
		cntUpdated = updateFromTempTable(tempTableName);
		truncateTempTable (tempTableName);
		gpcon2.closeAll();
	...
	{
		UpdateRecords ur = new UpdateRecords(args);
		boolean usegpfdist = ParseParameters.existsAttribute("-gpfdist", args);
		int cnt = ur.updateRecords(usegpfdist);
